...
class DataProvider {
  ...
  Stream<List<TaskList>> getLists(String idUser) {
    return FirebaseFirestore.instance.collection('task_lists').where('users', arrayContains: idUser).snapshots().map((snapshot) {
  ...
  Future<void> addTaskToTaskList(String listId, Task task) {
    FirebaseFirestore.instance.collection('task_lists').doc(listId).update({'tasks_quantity': FieldValue.increment(1)});
  ...
  Future<void> setTaskCompleted(String listId, String taskId, bool completed) {
    ...
    return FirebaseFirestore.instance.collection('task_lists').doc(listId).collection('tasks').doc(taskId).update({
      'completed': completed,
      'completed_by': DataProvider.instance.currentUser.uid,
    });
  }
  ...
  Future<void> deleteTask(String listId, String taskId) async {
    ...
    FirebaseFirestore.instance.collection('task_lists').doc(listId).update({
      ...
      'completed_tasks_quantity': task.getCompleted() ? FieldValue.increment(-1) : FieldValue.increment(0),
    });
  ...
  Future<Task> getTask(String listId, String taskId) async {
    final task = await FirebaseFirestore.instance.collection('task_lists').doc(listId).collection('tasks').doc(taskId).get();
    return Task.fromJson(task.data()!, task.id);
  }

  Future<void> deleteCompletedTasks(String listId) async {
    final tasksCollection = FirebaseFirestore.instance.collection('task_lists').doc(listId).collection('tasks');

    final lengthCompleted = await tasksCollection.where('completed', isEqualTo: true).get().then((value) => value.docs.length);

    final completedTasksQuery = tasksCollection.where('completed', isEqualTo: true);
  ...
  Future<void> addUsersToList(String listId, List<String> users) {
    return FirebaseFirestore.instance.collection('task_lists').doc(listId).update({'users': FieldValue.arrayUnion(users)});
  }
  ...
  Future<List<ShareableUser>> getUsersFromList(String listId) async {
    final users = await FirebaseFirestore.instance.collection('task_lists').doc(listId).get();
  ...
  Future<ShareableUser> getUser(String userId) {
    return FirebaseFirestore.instance.collection('users').doc(userId).get().then((doc) => ShareableUser.fromJson(doc.data()!, doc.id));
  }

  Future<String> getUserName(String userId) {
    return FirebaseFirestore.instance.collection('users').doc(userId).get().then((doc) => doc.data()!['name']);
  }
}
